...
const GamePage = () => {
  ...
  const [playerId, setPlayerId] = useState(user?._id);
  ...
  const createGame = async () => {
    try {
      const response = await axios.post(`${import.meta.env.VITE_BASE_URL}/game/create`, { creatorId: playerId });
      setGame(response.data);
      socket.emit("gameCreated", response.data);
    } catch (error) {
  ...
  const joinGame = async () => {
    if (!game?._id) return;
    try {
      const response = await axios.post(`${import.meta.env.VITE_BASE_URL}/game/join`, { gameId: game._id, playerId });
      setGame(response.data);
  ...
  const handleCardClick = async (cardId) => {
    ...
    if (newSelection.length === 2) {
      try {
        ...
        socket.emit("makeMove", { gameId: game._id, moveData: updatedGame });
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-800 p-4">
      ...
      {game && (
        <h2 className={`text-2xl font-semibold mb-4 ${game.currentTurn === playerId ? "text-green-500" : "text-red-500"}`}>
      ...
      {!game && (
        <button onClick={createGame} className="bg-green-500 text-white py-2 px-6 rounded-lg hover:bg-green-600">
          Create Game
        </button>
      )}
      {game && game.players.length < 2 && (
        <button onClick={joinGame} className="bg-purple-500 text-white py-2 px-6 rounded-lg hover:bg-purple-600 mt-4">
          Join Game
        </button>
      )}
      {game && (
        <div className="grid grid-cols-5 gap-4 mt-6">
          {game.cards.map((card) => (
            <div
              key={card.id}
              className={`w-24 h-24 bg-white rounded-lg shadow-md cursor-pointer transform transition-transform duration-500 ${card.matched || selectedCards.includes(card.id) ? "rotate-y-180 bg-green-300" : "bg-gray-300"}`}
